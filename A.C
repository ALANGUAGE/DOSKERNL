char Version1[]="Comp A.COM V1.2";//18630 bytes. 32002 stack
/*todo:
	...
	#ifdef debug, #endif
	CS:seg override [mem] OK, [es:2Ch] not OK 
*/
...
int printunsignedlong(unsigned long n) {
    ...
    c += '0';
    prc(c);//only print with isPrint=on
}
...
int getlex() {
    unsigned char c; unsigned char nc; char *p;
    char symboltmp[80];
    int i; int j; 
    long L;
  ...
  if (digit(c)) {
      ...
      if (thechar=='X') {
          ...
          while(alnum(thechar)) {
              c=next();
               if (c >= 'a') c -= 39;//conv. lowercase to capital letter
               if (c >= 'A') c -=  7;//A-F //c=c-7;
               c -= '0';
           ...
           while(digit(thechar)) {
               	c=next();
               	c -= '0';
               	//emulate: Llexval=Llexval*(long)10;
				Llexval;	//asm mov eax, [Llexval]
		    	ebx = 10;	//__emit__ (0x66,0xBB,0x0A,0,0,0 );
            	asm mul ebx
