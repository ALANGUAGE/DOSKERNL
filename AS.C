char Version1[]="ASM.C V1.3";//Assembler like NASM, 12593 bytes
...
unsigned long SymbolLong;//long value set in getDigit
...
unsigned int LabelMaxIx=0;	//actual # of stored labels. 1 to LABELADRMAX-1
unsigned int tmpLabelMaxIx; //set after PROC to LabelMaxIx
unsigned int LabelIx;       //actual # of just searched label
...
unsigned char FileBin  [FILEBINMAX];//output binary file
...
int getDigit(unsigned char c) {//ret: SymbolInt, SymbolLong
	unsigned long CastLong;
	unsigned char nextchar;
	unsigned char *NextInputPtr;
	SymbolLong = (long) 0;

	NextInputPtr = InputPtr + 1;
	nextchar = *NextInputPtr;
	if (nextchar == 'x') nextchar = 'X';	
	if ( nextchar == 'X') {
		InputPtr += 2;//advance x and get next c
		c = *InputPtr;						
		do {
			if (c>96) c-=39;//convert lowercase letter to capital letter      	       
			if (c>64) c=c-7;//A-F
			c -='0';
			SymbolLong = SymbolLong << 4;// *16
			CastLong = (long) c; //cast byte2double
			SymbolLong=SymbolLong+CastLong;	
			InputPtr++;
			c = *InputPtr;						
		} while(alnum(c));	
	} else {		
		do {
			c -='0';
			//simulate SymbolLong=SymbolLong*10;	
			SymbolLong;//mov eax, [SymbolLong]
			ebx = 10;
			asm mul ebx	
			asm mov dword [SymbolLong], eax
...
int genImmediate() {
    if (wflag) {
	    if (OpSize == DWORD) genCode32(imme32);
        else genCode16(imme);
	}else genCode8(imme);
}
...
int getTokeType() {
    ...
    if (letter (c)) {getName(c); TokeType=ALNUME; return;}//ret:2=Symbol
    TokeType=NOALNUME;
}
...
int getOpR() {
    ...
    if (Op2 == IMM)   {imme=SymbolInt; imme32=SymbolLong; return;}
    if (Op2 == REG)                            return;
    if (Op2 == ADR)   {
        if (LabelIx == 0) disp=0;
...
int getVariable() {
    ...
    if(TokeType==ALNUME) {//getName
        ...
        if (CodeType == 202) {//DD
            do {
                getTokeType();
                if (TokeType ==DIGIT) genCode32(SymbolLong);
            } while (isToken(','));
        }
...
int getarg() {
    ...
    *i=0;
    ...
    strcpy(namein, argv); strcat(namein, ".S");
    strcpy(namelst,argv); strcat(namelst,".LST");
    strcpy(namebin,argv); strcat(namebin,".COM");
